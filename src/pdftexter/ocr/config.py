...
class OllamaOCRConfig(BaseModel):
    ...
    base_url: str = Field(
        ...
        description="Ollama APIのベースURL（例: http://localhost:11434）",
    )
    ...
    max_retries: int = Field(3, description="最大リトライ回数")
    retry_delay: int = Field(5, description="リトライ間隔（秒）")
    language: Optional[str] = Field("Japanese", description="既定プロンプトの言語ヒント")
...
class OCRConfig(BaseModel):
    ...
    ocr_backend: str = Field(
        "deepseek",
        description="OCRバックエンド（deepseek or ollama）",
    )
    deepseek_ocr: DeepSeekOCRConfig
    ollama_ocr: OllamaOCRConfig = Field(default_factory=OllamaOCRConfig)
    output: OutputConfig
    ...
    def validate_ocr_backend(cls, v: str) -> str:
        ...
        if v not in ["deepseek", "ollama"]:
            raise ValueError("ocr_backend must be 'deepseek' or 'ollama'")
        return v
...
def get_default_config() -> OCRConfig:
    ...
    """
    return OCRConfig(
        ocr_backend=os.environ.get("PDFTEXTER_OCR_BACKEND", "deepseek"),
        deepseek_ocr=DeepSeekOCRConfig(
        ...
        ollama_ocr=OllamaOCRConfig(
            base_url=os.environ.get("OLLAMA_BASE_URL", "http://localhost:11434"),
            model_name=os.environ.get("OLLAMA_OCR_MODEL_NAME", "openbmb/minicpm-v4.5"),
            max_tokens=4096,
            temperature=0.1,
            output_format="markdown",
            timeout=300,
            max_retries=3,
            retry_delay=5,
            language=os.environ.get("OLLAMA_OCR_LANGUAGE", "Japanese"),
        ),
        output=OutputConfig(
            base_dir="./output",
